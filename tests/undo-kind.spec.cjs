...
const ROOT=require('path').resolve(__dirname,'..'),PORT=8360;
...
(async()=>{
  ...
  // apply carousel -> now undoable via setMode
  ...
  const canUndo = await page.evaluate(()=>window.__app.undo.canUndo());
  chk('applying carousel is now undoable', canUndo, 'canUndo='+canUndo);
  await page.click('#btnDone'); await page.waitForFunction(()=>window.__app.state.view==='overview',null,{timeout:5000});
  await page.waitForTimeout(150);
  const badgeBefore = await page.evaluate(()=>{const k=document.querySelector('#poStrip .po-card .po-kind');return k?k.textContent:null;});
  chk('card shows carousel badge', !!badgeBefore && /Carousel/i.test(badgeBefore), 'badge='+badgeBefore);
  const undoDisabled = await page.evaluate(()=>document.getElementById('btnUndo').disabled);
  chk('undo button enabled in overview', !undoDisabled, 'disabled='+undoDisabled);
  // UNDO in overview, NO tap after
  await page.click('#btnUndo');
  await page.waitForTimeout(300);
  const kindAfter = await page.evaluate(()=>window.__app.state.sources[0].kind);
  const badgeAfter = await page.evaluate(()=>{const k=document.querySelector('#poStrip .po-card .po-kind');return k?k.textContent:null;});
  chk('kind restored to plain by undo', kindAfter==='plain', 'kind='+kindAfter);
  chk('overview badge cleared IMMEDIATELY (no tap)', badgeAfter===null, 'badge still='+badgeAfter);
