...
export const importer = (function () {
    ...
    try {
      for (const f of files) {
        try {
          ...
          const id = nextId();
          state.sources.push({ id, name: f.name || (id + ".jpg"), bitmap, w, h, thumbUrl, blob: f, crop: null, croppedBitmap: null, cropDims: null, frame: cloneFrame(state.frame), kind: "plain", carousel: cloneCarousel(state.carousel), collage: cloneCollage(state.collage, true) });
  ...
  async function restoreSource(rec, crop) {
    ...
    state.sources.push({
      ...
      blob: rec.blob, crop: crop || null, croppedBitmap: null, cropDims: null, frame: cloneFrame(state.frame), kind: "plain", carousel: cloneCarousel(state.carousel), collage: cloneCollage(state.collage, true),
    });
